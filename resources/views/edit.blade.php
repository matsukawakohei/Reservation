    ...
    <div class="container-fluid text-center my-5">
        <div class='row'>
            <div class='col d-flex align-items-center justify-content-start'>
                <a class='h3' href={{ route('edit', ['id' => $reserve->id, 'start' => date('Y-m-d', strtotime($start .'-1 week'))]) }}>< 前の週へ</a>
            </div>
            <div class='col d-flex align-items-center justify-content-center'>
                <span class='h1'> 予約変更</span>
            </div>
            <div class='col d-flex align-items-center justify-content-end'>
                <a class='h3' href={{ route('edit', ['id' => $reserve->id, 'start' => date('Y-m-d', strtotime($start .'+1 week'))]) }}>次の週へ ></a>
            </div>
        </div>
    </div>
    <div class="container-fluid text-center my-5">
        <div class='row'>
            <div class='col d-flex align-items-center justify-content-center border h2 my-0'>
                予約日
            </div>
            <div class='col d-flex align-items-center justify-content-center border h2 my-0'>
                開始時間
        ...
        <div class='row'>
            <div class='col d-flex align-items-center justify-content-center border'>
                <span class='h3 my-0' id='old_date'>{{ date('Y/m/d', strtotime($reserve->start_time)) }}</span>
            </div>
            <div class='col d-flex align-items-center justify-content-center border'>
                <span class='h3 my-0' id='old_start'>{{ date('H:i', strtotime($reserve->start_time)) }}</span>
            </div>
            <div class='col d-flex align-items-center justify-content-center border'>
                <span class='h3 my-0' id='old_end'>{{ date('H:i', strtotime($reserve->end_time)) }}</span>
            </div>
        </div>
    </div>
